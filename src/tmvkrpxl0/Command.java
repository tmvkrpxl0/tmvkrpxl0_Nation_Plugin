	...
	public boolean onCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		...
		if(args.length > 0) {
									...
									if(TeamManager.getNation(Bukkit.getPlayerExact(args[1]))==null) {
										if(!PermissionManager.getPermission(Bukkit.getPlayer(args[1]), PermissionManager.create)) {
											if(TeamManager.invite(Bukkit.getPlayerExact(args[1]), TeamManager.getNation(p))) {
												sender.sendMessage(Core.prefix + args[1] + "님에게 성공적으로 초대를 보냈습니다!");
						...
						if(args.length > 1) {
							if(TeamManager.getNation(p).equals(TeamManager.getNation(Bukkit.getPlayerExact(args[1])))) {
								if(!args[1].equals(sender.getName())) {
									...
									}else sender.sendMessage("왕 또는 관리자는 추방될 수 없습니다!");
								}else {
			...
			case "탈퇴":
				if(TeamManager.getNation(p)!=null) {
					if(!PermissionManager.getPermission(p, PermissionManager.secondary)) {
						sender.sendMessage(Core.prefix + ChatColor.RED + "당신은 국가에서 탈퇴하셨습니다!");
						for(String s : TeamManager.getTeam(TeamManager.getNation(p))) {
							if(s.equals(p.getName()))continue;
							Bukkit.getPlayerExact(s).sendMessage(Core.prefix + sender.getName() + "님께서 국가에서 탈퇴하셨습니다!");
						}
			...
			case "전쟁선포":
				if(TeamManager.getNation(p)!=null) {
					if(PermissionManager.getPermission(p, PermissionManager.secondary)) {
						if(p.getInventory().containsAtLeast(Core.declarepaper, 1)) {
							if(TerritoryManager.getRegionNumber(TeamManager.getNation(p))>0) {
								if(args.length==1) {
									sender.sendMessage(Core.prefix + "전쟁선포를 할 국가를 입력하세요");
									sender.sendMessage(Core.prefix + "주의하세요. 전쟁 선포는 건쪽에서 취소할 수 없습니다!");
			...
			case "전쟁방어":
				if(TeamManager.getNation(p)!=null) {
					if(PermissionManager.getPermission(p, PermissionManager.secondary)) {
						if(p.getInventory().containsAtLeast(Core.defendpaper, 1)) {
							String from = TeamManager.getNation(p);
								...
								if(info.getReadyTime()>0) {
									if(info.getStarter().equals(from)) {
										...
										for(String pt : TeamManager.getTeam(BattleManager.getOpponent(from))) {
											Bukkit.getPlayerExact(pt).sendMessage(Core.prefix + from + "국이 전쟁 방어권을 사용해 전쟁을 취소시켰습니다!");
										}	
										for(org.bukkit.inventory.ItemStack item : p.getInventory()) {
											if(item.isSimilar(Core.defendpaper)) {
												if(item.getAmount()==1) {
			...
			case "설정":
				if(p.hasPermission("minecraft.command.op")) {
					if(args.length == 2) {
							switch(args[1]) {
								case "권한설정":
									sender.sendMessage(Core.prefix + "왕과 개척자 권한을 지정합니다");
								...
								default:
									...
									for(String s : settings) {
										if(!Core.patch && s.equals("패치"))continue;
								...
								if(Core.patch) {
									Thread t = new Thread() {
										public void run() {
												...
												Core.broadcast("앞으로 서버를 여실때는 _PATCHED.jar 로 끝나는 파일로 서버를 여시기 바랍니다.");
				...
				break;
			case "test":
				if(sender instanceof Player && p.hasPermission("minecraft.command.op")) {
					p.getInventory().addItem(Core.declarepaper);
					p.getInventory().addItem(Core.defendpaper);
				}
				break;
